 ...
 */
static char *find_next_arg(char *str, char ** const end, const unsigned terminate) {
    while (*str == ' ') {
        ++str;
    }
    if (*str == '\0') {
...
int main(const int argc, char ** const argv)
{
    ...
    if (debug_flag > 3) {
        ...
        for (unsigned i = 1; i <= 14; i++) {
            const unsigned clk = mbox_get_clock_rate(i);
            if (clk) {
                kprintf(" %u=%uMHz", i, clk / 1000000);
            }
        }
        kprintf("\n");
        kprintf("Power");
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/hardware/startup/boards/bcm2711/main.c $ $Rev: 984465 $")
#endif
