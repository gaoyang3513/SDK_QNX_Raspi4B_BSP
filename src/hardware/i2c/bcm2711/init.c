...
void *bcm2711_init(int argc, char *argv[])
{
    ...
    if (!dev) {
        slogf(_SLOG_SETCODE(_SLOGC_CHAR, 0), _SLOG_ERROR, "%s:Failed to allocate memory", __FUNCTION__);
        return NULL;
    }
...
int bcm2711_attach_intr(bcm2711_dev_t *dev)
{
    ...
    if (dev->iid == -1) {
        bcm2711_i2c_slogf(dev, _SLOG_ERROR,"%s: Failed to attach to interrupt", __FUNCTION__);
        return -1;
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/hardware/i2c/bcm2711/init.c $ $Rev: 960373 $")
#endif
